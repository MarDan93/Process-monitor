...
with tab3:
    if st.session_state.df_X is None:
        ...
        # ── Mostra sempre i risultati dal session_state ──────────
        if st.session_state.model is not None:
            ...
            x_cols_model=model['feature_names']
            flagged_p1=np.where(fT2|fQ)[0]
            if len(flagged_p1)>0:
                st.markdown("#### Contribution plots — cicli anomali Phase I")
                st.caption(f"{len(flagged_p1)} cicli fuori controllo nel train set.")

                with st.expander("📋 Indice variabili",expanded=False):
                    st.dataframe(
                        pd.DataFrame({'Indice':range(1,len(x_cols_model)+1),
                                      'Variabile':x_cols_model}),
                        use_container_width=True,hide_index=True)

                p_count=len(x_cols_model)
                ...
                sel_p1=st.dataframe(df_flag_p1,use_container_width=True,
                                    hide_index=True,on_select='rerun',
                                    selection_mode='single-row',key='p1_flag_table')
                ...
                t2_p1=float(model['T2'][obs_p1]); q_p1=float(model['Q'][obs_p1])
                st.markdown(
                    f"<div class='alarm-box'><strong>Ciclo {obs_p1} (Phase I)</strong><br>"
                    f"T²={t2_p1:.3f} ({t2_p1/model['T2_UCL']:.2f}×UCL) | "
                    f"Q={q_p1:.3f} ({q_p1/model['Q_UCL']:.2f}×UCL)</div>",
                    unsafe_allow_html=True)

                Xs_p1=model['X_scaled']; E_p1=model['E']
                T_p1=model['scores']; lam_p1=model['eigenvalues']; P_p1=model['loadings']
                ...
                with col_l:
                    fig_ct2_p1,exc_t2_p1=make_contribution_chart(
                        ...
                        x_cols_model,f'T² Contribution — Ciclo {obs_p1} (Phase I)')
                    st.plotly_chart(fig_ct2_p1,use_container_width=True,
                                    key=f'p1_ct2_{obs_p1}')
                    df_ct2_p1=pd.DataFrame({
                        'Idx':range(1,p_count+1),'Variabile':x_cols_model,
                    ...
                    if exc_t2_p1:
                        ...
                        st.dataframe(pd.DataFrame(exc_t2_p1,
                ...
                with col_r:
                    fig_cq_p1,exc_q_p1=make_contribution_chart(
                        ...
                        x_cols_model,f'Q Contribution — Ciclo {obs_p1} (Phase I)')
                    ...
                    if exc_q_p1:
                        ...
                        st.dataframe(pd.DataFrame(exc_q_p1,
                                     columns=['Idx','Variabile','Valore','LCL','UCL']),
                                     use_container_width=True,hide_index=True)

                st.markdown("---")
                v_t2_p1=[x_cols_model[i] for i in top_t2_p1]
                v_q_p1 =[x_cols_model[i] for i in top_q_p1]
                prompt_p1=f"""Sei un esperto di SPC multivariato per stampaggio a iniezione.
Ciclo anomalo {obs_p1} nel training set: T²={t2_p1:.3f} ({t2_p1/model['T2_UCL']:.2f}×UCL), Q={q_p1:.3f} ({q_p1/model['Q_UCL']:.2f}×UCL).
Variabili T²: {', '.join(v_t2_p1)}. Variabili Q: {', '.join(v_q_p1)}.
Spiega in italiano a un process engineer:
...
Modello PCA-SPC: {len(model['T2'])} cicli, k={model['k']} PC, T² UCL={model['T2_UCL']:.3f}, Q UCL={model['Q_UCL']:.3f}, α={alpha}.
Flag residui: {n_flag} ({pct_f:.1f}%). Variabili: {', '.join(model['feature_names'][:8])}{'...' if len(model['feature_names'])>8 else ''}
Spiega in italiano cosa rappresentano T² UCL e Q UCL, se i flag residui sono accettabili e come interpretare i grafici."""
    ...
    if st.session_state.model is None:
        ...
        if up_test:
            ...
            if miss_c:
                ...
                # Salva nel session_state — così sopravvive ai rerun
                st.session_state.mon=mon
                st.session_state.df_test_X=df_test_X

        # Leggi sempre dal session_state — non dall'uploader
        if st.session_state.mon is not None and st.session_state.df_test_X is not None:
            mon=st.session_state.mon
            df_test_X=st.session_state.df_test_X
...
Spiega in italiano a un capoturno cosa sta succedendo al processo e se c'è motivo di preoccupazione. Max 150 parole."""
